...
public class FightState extends InWorldState{
	...
	private void drawEntities(Graphics2D g2) {
		...
		if(enemy.name.equals("Lord JJ")) {
			g2.drawImage(enemy.getIdle(), enemyRect.x - 200, enemyRect.y - 190, enemyRect.width + 150, enemyRect.height + 200, null);
		}
		else {
	...
	private void callSkill(Graphics g) {
		...
		g.drawImage(Images.tint(enemy.getIdle(),0,0,Math.max(0, blue--)),enemyRect.x - 200, enemyRect.y - 190, enemyRect.width + 150, enemyRect.height + 200,null);

		if(enemy.name.equals("Lord JJ")) {
		g.drawImage(playerIceSkill.getCurrentFrame(),(handler.getWidth() * 4/ 5)-350,entityY - 300,500,500,null);
		}
		else {
